...
public class LocomotiveSourceDQ1Pane implements UIFormPane {
	...
	public void writePane(UIFormPaneWriter pWriter, UIFormContext aContext) {
		...
		String query = "Select s.\"$adaptation\" from \"/root/Loco_Sources_Second\" s where FK_AS_STRING('loco_data','/root/Loco_Sources_Second', s.Loco_Number) = '"
				+ locoNumber + "'";
		...
		for (Tuple result : sourceRecords) {
			...
			locoSourceString = sourceRecord
					.getString(Paths._Root_Loco_Sources_Second._Root_Loco_Sources_Second_Loco_Sources);
			locoNumberString = locoNumber.toString();
			overallDQScore = sourceRecord
					.getString(Paths._Root_Loco_Sources_Second._Root_Loco_Sources_Second_Overall_DQ_Score);
			dqScore = sourceRecord.getString(Paths._Root_Loco_Sources_Second._Root_Loco_Sources_Second_DQ_Score);
			locoTypeString = sourceRecord
					.getString(Paths._Root_Loco_Sources_Second._Root_Loco_Sources_Second_Loco_Type);
			pWriter.add("<tr>");
			if (displayFlag == ((printFlag / 2) + 1)) {
				pWriter.add(
						"<tr><td style='white-space: nowrap; color: gray; border-left: 1px solid; text-align:center;'>"
								+ locoNumberString + "</div></td>");
				pWriter.add(
						"<td style='white-space: nowrap; color: gray; border-left: 1px solid; border-right: 1px solid; text-align:center;'>"
								+ overallDQScore + "</div></td>");
				printFlag = printFlag + 2;
			} else {
				System.out.println("==Count Bottom ===" + count + "---" + countBottomBorder);
				if (count == countBottomBorder) {
					pWriter.add(
							"<tr><td style='white-space: nowrap; color: gray; border-left: 1px solid; border-bottom: 1px solid;'>"
									+ "" + "</div></td>");
					pWriter.add(
							"<td style='white-space: nowrap; color: gray; border-left: 1px solid; border-right: 1px solid; border-bottom: 1px solid;'>"
									+ "" + "</div></td>");
					System.out.println("===bottom border===");
				} else {
					pWriter.add("<tr><td style='white-space: nowrap; color: gray; border-left: 1px solid; '>" + ""
							+ "</div></td>");
					pWriter.add(
							"<td style='white-space: nowrap; color: gray; border-left: 1px solid; border-right: 1px solid;'>"
									+ "" + "</div></td>");
				}

			}
			pWriter.add("<td style='white-space: nowrap; color: gray; border: 1px solid;'>" + locoSourceString
					+ "</div></td>");

			pWriter.add("<td style='white-space: nowrap; color: gray; border: 1px solid;'>" + dqScore + "</div></td>");

			pWriter.add("<td style='white-space: nowrap; color: gray; border: 1px solid;'>" + locoTypeString
					+ "</div></td>");
