...
public class LocomotiveSourcesPaneTest implements UIFormPane {
	...
	public void writePane(UIFormPaneWriter pWriter, UIFormContext aContext) {
		...
		String query = "Select s.\"$adaptation\" from \"/root/Loco_Sources\" s where FK_AS_STRING('loco_data','/root/Loco_Sources', s.Loco_Number) = '"
				+ locoNumber + "'";
		...
		for (Tuple result : sourceRecords) {
			...
			System.out.println("===loco mfg==="
					+ sourceRecord.getString(Paths._Root_Loco_Sources._Root_Loco_Sources_Loco_Manufacturer));
			System.out.println(
					"===loco type===" + sourceRecord.getString(Paths._Root_Loco_Sources._Root_Loco_Sources_Loco_Type));
			...
			locoManufacturerString = sourceRecord
					.getString(Paths._Root_Loco_Sources._Root_Loco_Sources_Loco_Manufacturer);
			locoOwningZoneString = sourceRecord.getString(Paths._Root_Loco_Sources._Root_Loco_Sources_Loco_Owning_Zone);
			locoOwingDivisionString = sourceRecord
					.getString(Paths._Root_Loco_Sources._Root_Loco_Sources_Loco_Owning_Division);
			locoOwningShedString = sourceRecord.getString(Paths._Root_Loco_Sources._Root_Loco_Sources_Loco_Owning_Shed);

			pWriter.add("<tr><td style='white-space: nowrap; color: gray; border: 1px solid; background: green;'>"
			...
			pWriter.add("<td style='white-space: nowrap; color: gray; border: 1px solid;'>" + locoManufacturerString
					+ "</div></td>");

			pWriter.add("<td style='white-space: nowrap; color: gray; border: 1px solid;'>" + locoOwningZoneString
					+ "</div></td>");

			pWriter.add("<td style='white-space: nowrap; color: gray; border: 1px solid;'>" + locoOwingDivisionString
					+ "</div></td>");

			pWriter.add("<td style='white-space: nowrap; color: gray; border: 1px solid;'>" + locoOwningShedString
					+ "</div></td>");
