...
public class LocoUpdateDaaService implements UserService<TableViewEntitySelection> {

	private WorkItemKey wiKey;
	private DisplayStep currentStep = null;

	public LocoUpdateDaaService() {
		super();
	...
	public UserServiceEventOutcome processEventOutcome(
		...
		try {
			...
			launcher = wfEngine.getProcessLauncher(PublishedProcessKey.forName("loco_update_ui_daa"));
			...
			if (result.isSizeGreaterOrEqual(1)) {
				...
				launcher.setInputParameter("locoNumber", locoNumber);
			} else
	...
	public void setupObjectContext(UserServiceSetupObjectContext<TableViewEntitySelection> context,
		...
		if (LocoRecordOperationRestrictionUtils.checkRunningProcesses(wfEngine, "loco_create_api", "record",
			...
			this.currentStep = new DisplayExitMessageScreen(" The selected record Loco Number = " + locoNumber
					+ " has already an operation (Loco SLAM API) in process.");
			return;
		}
		...
		if (LocoRecordOperationRestrictionUtils.checkRunningProcesses(wfEngine, "loco_update_ui_daa", "record",
				record.toXPathExpression())) {
			this.currentStep = new DisplayExitMessageScreen(" The selected record Loco Number = " + locoNumber
					+ " has already an operation (DAA Loco Update) in process.");
			return;
		}
