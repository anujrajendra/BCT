...
public class LocoValidationReportForSingleAttribute extends ScheduledTask {
	...
	public void execute(ScheduledExecutionContext arg0) throws OperationException, ScheduledTaskInterruption {
		...
		while (validationReportItemIterator.hasNext()) {
			...
			try {
				if (validationReportItem.toString().contains(attributeName)) {
					...
					if (validationReportItemSubjectForAdaptation != null) {
						...
						String locoId = (String) validationRecord.get(Paths._Root_Loco_Type._Root_Loco_Type_Loco_Type);
	...
	private String readValidationReportFileName() {
		...
		return properties.getProperty("ebx.loco.loco.validationreportfileforsingleattribute");
