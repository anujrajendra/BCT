...
public class LocoTransferService implements UserService<TableViewEntitySelection> {
	...
	public void setupObjectContext(UserServiceSetupObjectContext<TableViewEntitySelection> context,
		...
		if (oldShed != null)
			shedRecord = shedTable.lookupAdaptationByPrimaryKey(PrimaryKey.parseString(oldShed));
		...
		if (sourceSystemName != null)
			if (sourceSystemName.equalsIgnoreCase("SLAM")) {
				this.currentStep = new DisplayResultStep(
						" cannot be processed. Please use SLAM application to transfer Locomotive records for the selected shed");
				return;
			}
