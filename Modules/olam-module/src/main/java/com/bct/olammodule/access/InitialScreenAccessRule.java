...
public class InitialScreenAccessRule implements AccessRule{
	...
	public AccessPermission getPermission(final Adaptation adaptation, Session session, SchemaNode node) {
		...
		String jointKey = commodityString +"|"+ businessMaterialTypeString + "|" + procurementPk +"|"+ plantPk +"|"+ "Initial screen";
...
//		System.out.println("====Wrapper access permission string===="+
//				wrapper.masterViewMatrix(plantPk, commodityString, businessMaterialTypeString, procurementPk, "Initial screen"));
		if (session.getUserReference().getUserId().equalsIgnoreCase("admin"))
			return AccessPermission.getReadWrite();
		
		System.out.println("==User in role Production Team==="+session.isUserInRole(Role.forSpecificRole("RProduction Team")));
		...
		if(session.isUserInRole(Role.forSpecificRole("RProduct Development Team")) || 
			...
			return AccessPermission.getReadWrite();
		else if(session.isUserInRole(Role.forSpecificRole("RProduction Team")) || 
				session.isUserInRole(Role.forSpecificRole("RQuality Team")) ||
				session.isUserInRole(Role.forSpecificRole("RWarehouse Team")) ||
				session.isUserInRole(Role.forSpecificRole("RFinance Team")) 
				)
		...
		return AccessPermission.getReadWrite(); // parseFlag("readWrite");
